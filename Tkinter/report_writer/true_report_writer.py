...
def report_writer():
    try: 
        ...
        def clear(frame):
            global uni_df, df_add
            uni_df.drop(index=uni_df.index, inplace=True)
            df_add.drop(index=df_add.index, inplace=True)

            history_label.config(text="No Records")
            hour_history.config(text="")
            minute_history.config(text="")
            desc_history.config(text="")
        ...
        if getattr(sys, "frozen", False):
            icon_path = os.path.join(sys._MEIPASS, 'report_document_finance_business_analysis_analytics_chart_icon_188615.ico')
        else: 
            icon_path = r"C:\Users\samil\Downloads\report_document_finance_business_analysis_analytics_chart_icon_188615.ico"
        ...
        # Enter button
        enter_button = Button(frame1, text="Enter", command=new_df)
        ...
        # Placements
        history_frame.grid(row=1, column=0, padx=30, pady=30, columnspan=3, sticky="nsew")
        ...
        clear_button = Button(main_frame, text="Clear", command=lambda: clear(history_frame), width=10, height=1)
